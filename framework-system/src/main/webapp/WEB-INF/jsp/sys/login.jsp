...
<%-- 				<c:if test="${param.error != null && param.error != ''}"> --%>
				  ...
				  <c:when test="${SPRING_SECURITY_LAST_EXCEPTION.message eq 'Bad credentials'}">   
				    <span><font color="red">密码错误</font></span>
				  </c:when> 
				  <c:when test="${SPRING_SECURITY_LAST_EXCEPTION.message eq 'loginname is null...'}">   
				    <span><font color="red">用户名不可以为空</font></span>  
				  </c:when> 
				  <c:when test="${SPRING_SECURITY_LAST_EXCEPTION.message eq 'loginname is null'}">   
				    <span><font color="red">密码不可以为空</font></span>  
				  </c:when> 
				  <c:otherwise>   
				    <span><font color="red">用户名不存在</font></span>
				  </c:otherwise> 
				</c:choose> 
<%-- 				</c:if> --%>
					<form action="${pageContext.request.contextPath }/login" method="post">
						<input name="username" value="${SPRING_SECURITY_LAST_USERNAME}" type="text" placeholder="用户名" />
...
<!-- 					<script> -->
<!-- 					alert("${SPRING_SECURITY_LAST_EXCEPTION.message}")</script> -->
				</div>
			</div>
		</div>
		<div class="footer">
