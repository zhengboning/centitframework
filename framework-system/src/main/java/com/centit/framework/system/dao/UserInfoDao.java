...
/*        
    ...
    @SuppressWarnings("unchecked")
    @Transactional
    public List<UserInfo> listUserinfoByUsercodes(List<String> userCodes) {     
        
        return (List<UserInfo>)  
                this.getCurrentSession().createCriteria(UserInfo.class).
                    add( Restrictions.or(Restrictions.in(CodeRepositoryUtil.USER_CODE, userCodes),
                            Restrictions.in("loginname", userCodes))).list();
/*        return (List<UserInfo>)super.getHibernateTemplate().findByCriteria(
                DetachedCriteria.forClass(UserInfo.class).add(
                        Restrictions.or(Restrictions.in(CodeRepositoryUtil.USERCODE, userCodes),
                                Restrictions.in("loginname", userCodes))));*/
    }

    @SuppressWarnings("unchecked")
    @Transactional
    public List<UserInfo> listUserinfoByLoginname(List<String> loginnames) {
        return (List<UserInfo>)  
                this.getCurrentSession().createCriteria(UserInfo.class).
                    add(Restrictions.in("loginname", loginnames)).list();
