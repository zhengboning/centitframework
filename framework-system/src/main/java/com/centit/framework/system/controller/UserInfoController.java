...
public class UserInfoController extends BaseController {
    ...
    public void create(@Valid UserInfo userInfo,HttpServletRequest request, HttpServletResponse response) {
    	...
    	UserInfo dbuserinfo=sysUserManager.getObjectByProperty("loginName", userInfo.getLoginName());
    	if(null!=dbuserinfo)
    	{
    		 JsonResultUtils.writeErrorMessageJson(400, "登录名"+userInfo.getLoginName()+"已存在，请更换！", response);
    		 return;
    	}
    ...
    public void isExists(@PathVariable String loginName, 
        ...
        UserInfo userInfo = sysUserManager.getObjectByProperty("loginName", loginName);
    ...
    public void canChangePwd(@PathVariable String userCode,@PathVariable String oldPassword,HttpServletRequest request,HttpServletResponse response) {
        boolean bo=true;
        bo=sysUserManager.rightPW(userCode,oldPassword);
    ...
    /**
     * 对用户信息进行模糊搜索，适用于带搜索条件的下拉框。
     *
     * @param key      搜索条件
     * @param field    需要搜索的字段，如为空，默认，userCode,loginName,userWord(用户姓名拼音)
     * @param response HttpServletResponse
     */
    ...
    public void search(@PathVariable String key, String[] field, HttpServletResponse response) {
        if (ArrayUtils.isEmpty(field)) {
            field = new String[]{"userCode", "loginName", "userWord"};
        }

        List<UserInfo> listObjects = sysUserManager.search(key, field);
